...
exchange = ccxt.bitget({
    ...
    'password': os.getenv('BITGET_PASSWORD'),  # Some Bitget endpoints require a password/passphrase
...
async def buy_pi(current_price):
    ...
    try:
        ...
        # Place market buy order with Bitget
        exchange.load_markets()  # Ensure markets are loaded
...
async def sell_pi(current_price, reason="profit"):
    ...
    try:
        ...
        # Place market sell order with Bitget
...
async def trading_loop():
    ...
    await send_telegram_message(
        f"{random.choice(JARVIS_GREETINGS)}\n\nMonitoring {SYMBOL} for high-risk opportunities on Bitget. Safety protocols minimized as requested, sir.")
...
async def start_command(update, context):
    ...
    await update.message.reply_text(
        "JARVIS trading protocol activated, sir. Monitoring PI/USDT for aggressive trading opportunities on Bitget exchange."
    )
...
async def main():
    """Run the bot"""
    # Setup logging
    logging.info("Starting PI trading bot on Bitget")
