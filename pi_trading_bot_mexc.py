...
async def get_market_data():
    ...
    try:
        ...
        return {
            ...
            'change_24h': ticker['percentage'] if 'percentage' in ticker else 0,
            'high': ticker['high'],
            'low': ticker['low']
        }
...
async def analyze_market():
    ...
    try:
        # MEXC uses different timeframe format
        ...
        sma_short = sum(closes[-5:]) / 5 if len(closes) >= 5 else 0
        sma_long = sum(closes[-15:]) / 15 if len(closes) >= 15 else 0

        # Calculate risk score (higher = more favorable for buying)
        risk_score = 0

        # Trend analysis
        if sma_short > sma_long and sma_long > 0:
            risk_score += 3
...
async def buy_pi(current_price):
    ...
    try:
        ...
        # MEXC specific: Ensure amount precision is correct
        market = exchange.market(SYMBOL)
        if 'precision' in market and 'amount' in market['precision']:
            precision = market['precision']['amount']

            # Simpler approach to truncate to specific decimal places
            # Convert precision to decimal places (if it's a number)
            if isinstance(precision, (int, float)):
                decimal_places = int(precision)
                # Format to that many decimal places and truncate
                formatted_amount = str(int(amount * (10 ** decimal_places)) / (10 ** decimal_places))
                amount = float(formatted_amount)
            else:
                # If precision is not a number, just use the amount as is
                logger.warning(f"Precision format not recognized: {precision}, using original amount")
    ...
    try:
        ...
        if 'precision' in market and 'amount' in market['precision']:
            ...
            if isinstance(precision, (int, float)):
                decimal_places = int(precision)
                # Format to that many decimal places and truncate
                formatted_amount = str(int(pi_amount * (10 ** decimal_places)) / (10 ** decimal_places))
                pi_amount = float(formatted_amount)
            else:
                # If precision is not a number, just use the amount as is
                logger.warning(f"Precision format not recognized: {precision}, using original amount")
...
async def main():
    """Run the bot"""
    # Setup logging
    logging.info("Starting PI trading bot on MEXC")
